...
open class WsdlStatement(
    ...
    // Field to store the fetch size (page size) - configurable via environment
    private var fetchSize: Int = getDefaultFetchSize()
    
    companion object {
        // Maximum allowed fetch size to prevent memory issues
        private const val MAX_FETCH_SIZE = 10000
        
        // Minimum fetch size for reasonable performance
        private const val MIN_FETCH_SIZE = 1
        
        /**
         * Get default fetch size from environment or use 500 as fallback
         */
        private fun getDefaultFetchSize(): Int {
            val envFetchSize = System.getenv("OFJDBC_DEFAULT_FETCH_SIZE")?.toIntOrNull()
                ?: System.getProperty("ofjdbc.fetchSize")?.toIntOrNull()
                ?: 500
            return validateFetchSize(envFetchSize, "default")
        }
        
        /**
         * Validate and normalize fetch size value
         */
        fun validateFetchSize(size: Int, context: String): Int {
            return when {
                size < 0 -> throw SQLException("Fetch size cannot be negative: $size")
                size == 0 -> 500 // 0 means use default, avoid recursion
                size > MAX_FETCH_SIZE -> {
                    LoggerFactory.getLogger(WsdlStatement::class.java)
                        .warn("Fetch size {} exceeds maximum {}, capping at maximum (context: {})", 
                            size, MAX_FETCH_SIZE, context)
                    MAX_FETCH_SIZE
                }
                size < MIN_FETCH_SIZE -> {
                    LoggerFactory.getLogger(WsdlStatement::class.java)
                        .warn("Fetch size {} below minimum {}, using minimum (context: {})", 
                            size, MIN_FETCH_SIZE, context)
                    MIN_FETCH_SIZE
                }
                else -> size
            }
    ...
    override fun executeQuery(sql: String): ResultSet {
        ...
        /* --------------------------------------------------------------------
           ...
           -------------------------------------------------------------------- */
        val effectiveFetchSize = getEffectiveFetchSize()
        logger.debug("Executing query with effective fetch size: {} (requested: {})", 
    ...
    /**
     * Sets the fetch size hint for ResultSet objects generated by this Statement.
     * DBeaver and other JDBC clients use this to control pagination.
     */
    override fun setFetchSize(rows: Int) {
        validateOpen()
        val oldFetchSize = fetchSize
        fetchSize = Companion.validateFetchSize(rows, "setFetchSize")
        
        if (fetchSize != oldFetchSize) {
            logger.info("Fetch size changed from {} to {} (requested: {})", 
                oldFetchSize, fetchSize, rows)
        }
        
        // If the value was capped or adjusted, add a warning
        if (fetchSize != rows && rows != 0) {
            addWarning(SQLWarning("Fetch size adjusted from $rows to $fetchSize"))
        }
    }
    
    /**
     * Returns the current fetch size setting
     */
    override fun getFetchSize(): Int {
        validateOpen()
        return fetchSize
    ...
    /**
     * Calculate the effective fetch size to use for query execution.
     * This handles special cases and applies business logic.
     */
    private fun getEffectiveFetchSize(): Int {
        return when {
            fetchSize == 0 -> Companion.getDefaultFetchSize() // 0 means use default
            else -> fetchSize
        }
    }
